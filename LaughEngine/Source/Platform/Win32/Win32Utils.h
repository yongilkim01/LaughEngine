...
namespace Win32 
{
    ...
    namespace Utils 
    {
        BOOL LAUGH_API AddBitmap(CONST WCHAR* szFileName, HDC hWinDC, INT x = 0, INT y = 0);
        ...
        inline VOID LAUGH_API ModifyWindowExStyle(HWND hWnd, DWORD flagsToDisable, DWORD flagsToEnable)
        {
        ...
        inline BOOL LAUGH_API HasStyle(HWND hwnd, DWORD style) {
        ...
        /// Sets and clears style flags for a particular window.
        inline VOID LAUGH_API ModifyClassStyle(HWND hWnd, DWORD flagsToDisable, DWORD flagsToEnable)
        {
            DWORD style = GetWindowLong(hWnd, GCL_STYLE);
            SetClassLong(hWnd, GCL_STYLE, (style & ~flagsToDisable) | flagsToEnable);
        }

        inline BOOL LAUGH_API IsWindowFullscreen(HWND hWnd)
        {
        ...
        inline VOID LAUGH_API MaximizeWindow(HWND hwnd) {
