...
namespace Win32
{
	...
	VOID Window::Initialize()
	{
		...
		m_Handle = CreateWindow(m_Class.c_str(), m_Title.c_str(),
			m_Type, ((desktop.right / 2) - (Size().cx / 2)), ((desktop.bottom / 2) - (Size().cy / 2)),
	...
	LRESULT Window::MessageHandler(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_PAINT: { OnPaint(); }	break; // change display size case
	...
	VOID Window::OnNonClientCreate() {
		...
		Win32::Caption::AddCaptionButton(new CaptionButton(L"ㅁ", CB_MAXIMIZE));
		Win32::Caption::AddCaptionButton(new CaptionButton(L"_", CB_MINIMIZE));
	}
	...
	VOID Window::PaintCaption(HDC hdc) {
		...
		for (auto& button : Caption::CaptionButtons()) {
			...
			if (button->Text.compare(L"ㅁ") == 0 && Win32::Utils::IsWindowFullscreen(Handle())) 
			{
				button->Text = L"🗗";
			}
